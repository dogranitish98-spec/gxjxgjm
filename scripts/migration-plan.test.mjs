...
test("the auth schema ships outside the globbed directory", () => {
  ...
  // Research/paper migrations live at migrations/*.sql; the auth schema stays under migrations/auth/.
  const names = pendingMigrations(readdirSync(migrationsDir), []).map((m) => m.name);
  assert.ok(names.includes("0002_research_history.sql"));
  assert.ok(names.includes("0003_paper_trades_history.sql"));
  assert.ok(names.includes("0004_drop_paper_trade_links.sql"));
  assert.ok(!names.includes("0001_auth.sql"), "auth schema must stay outside the globbed directory");
